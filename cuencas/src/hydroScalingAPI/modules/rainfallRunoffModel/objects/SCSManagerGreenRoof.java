...
public class SCSManagerGreenRoof {
    ...
    public SCSManagerGreenRoof(java.io.File DemData,java.io.File LandUse,java.io.File SoilData,java.io.File Build, hydroScalingAPI.util.geomorphology.objects.Basin myCuenca, hydroScalingAPI.util.geomorphology.objects.LinksAnalysis linksStructure, hydroScalingAPI.io.MetaRaster metaDatos, byte[][] matDir, int[][] magnitudes) {
        ...
        System.out.println("DEM File = " + DemData +"\n");
     ...
     //       System.out.println("CREATE THE META RASTER _ SOIL DATA \n");
            ...
            HillslopeReliefArea =new double[linksStructure.contactsArray.length][5]; //[link][class]
            ...
            hillclasses = new double[linksStructure.contactsArray.length][5]; //[link][class]
            HillslopeRelief= new double[linksStructure.contactsArray.length]; //[link]
...
///////////////////// land use ///////////////////
                        ...
                        if(dataSnapShotLC[MatYLC][MatXLC]<10) currentHillBasedLandUse[matrizPintada[j][k]-1][1]++;
                        if(dataSnapShotLC[MatYLC][MatXLC]==2000) currentHillBasedLandUse[matrizPintada[j][k]-1][2]++;
                        if(dataSnapShotLC[MatYLC][MatXLC]==101) currentHillBasedLandUse[matrizPintada[j][k]-1][10]++; // Greenroof will be considered as grassland
///////////////////// Hyd_group ///////////////////
                        ...
                        if(dataSnapShotSOIL[MatYSOIL][MatXSOIL]==5) currentSoilType[matrizPintada[j][k]-1][4]++;
                        if(dataSnapShotSOIL[MatYSOIL][MatXSOIL]==6) currentSoilType[matrizPintada[j][k]-1][5]++;
...
///////////////////// K NRCS for the hillslope ///////////////////
                       ...
                       if(PixelMan>maxHillBasedMan[matrizPintada[j][k]-1]) maxHillBasedMan[matrizPintada[j][k]-1]=PixelMan;
                       if(PixelMan<minHillBasedMan[matrizPintada[j][k]-1] || PixelMan>0) minHillBasedMan[matrizPintada[j][k]-1]=PixelMan;

                       currentHillBased_K_NRCS[matrizPintada[j][k]-1]=currentHillBased_K_NRCS[matrizPintada[j][k]-1]+PixelK_NRCS;
                       if(PixelK_NRCS>maxHillBasedMan[matrizPintada[j][k]-1]) maxHillBasedMan[matrizPintada[j][k]-1]=PixelK_NRCS;
...
///////////////////// land use ///////////////////
   if(PixelH>maxHillBasedH[matrizPintada[j][k]-1]) maxHillBasedH[matrizPintada[j][k]-1]=PixelH;
   if(PixelH<minHillBasedH[matrizPintada[j][k]-1] && PixelH>0) minHillBasedH[matrizPintada[j][k]-1]=PixelH;
...
///////////////////// land use ///////////////////
   ...
   if(PixelH<=hillclasses[matrizPintada[j][k]-1][0]) HillslopeReliefArea[matrizPintada[j][k]-1][0]++;
   if(PixelH>hillclasses[matrizPintada[j][k]-1][0] && PixelH<=hillclasses[matrizPintada[j][k]-1][1]) HillslopeReliefArea[matrizPintada[j][k]-1][1]++;
   if(PixelH>hillclasses[matrizPintada[j][k]-1][1] && PixelH<=hillclasses[matrizPintada[j][k]-1][2]) HillslopeReliefArea[matrizPintada[j][k]-1][2]++;
   if(PixelH>hillclasses[matrizPintada[j][k]-1][2] && PixelH<=hillclasses[matrizPintada[j][k]-1][3]) HillslopeReliefArea[matrizPintada[j][k]-1][3]++;
   if(PixelH>hillclasses[matrizPintada[j][k]-1][3]) HillslopeReliefArea[matrizPintada[j][k]-1][4]++;
 ...
 for (int j=0;j<linksStructure.contactsArray.length;j++)
    {for (int ih=0;ih<5;ih++)
     {HillslopeReliefArea[j][ih]=100*HillslopeReliefArea[j][ih]/currentHillNumPixels[j];}
}
    ...
    {double PixelCN=-9.9;
        ...
        if(soil==1)
            {if(LC==82) PixelCN=64;
                    ...
                    LC==81 || LC==61 )                       PixelCN=39;
            else if(LC==21 || LC==83 || LC==84 || LC==85 ) PixelCN=51;
        ...
        if(soil==2)
            {if(LC==82) PixelCN=75;
                     ...
                     LC==71 || LC==72 || LC==73 || LC==74 ||
                     LC==81 || LC==61 )                     PixelCN=61;
             else if(LC==21 || LC==83 || LC==84 || LC==85 ) PixelCN=68;
             else if(LC==22) PixelCN=72;
             else if(LC==23) PixelCN=75;
             else if(LC==24) PixelCN=85;
             else if(LC==101) PixelCN=84; // GREENROOF
             else if(LC==11 || LC==12 || LC==90 || LC==95 || LC==92)PixelCN=100;
             else PixelCN=45;
             }
        ...
        if(soil==3)
           {if(LC==82) PixelCN=82;
                    ...
                    LC==71 || LC==72 ||LC==73 || LC==74 ||
                    LC==81 || LC==61 )PixelCN=74;
            else if(LC==21 || LC==83 ||LC==84 || LC==85 ) PixelCN=79;
            else if(LC==22) PixelCN=81;
            else if(LC==23) PixelCN=83;
            else if(LC==24) PixelCN=90;
            else if(LC==101) PixelCN=84; // GREENROOF
            else if(LC==11 || LC==12 || LC==90 || LC==95 || LC==92)PixelCN=100;
                        else PixelCN=50;
           }
        if(soil==4)
           {if(LC==82) PixelCN=85;
                    ...
                    LC==71 || LC==72 ||LC==73 || LC==74 ||
                    LC==81 || LC==61 )PixelCN=80;
            else if(LC==21 || LC==83 ||LC==84 || LC==85 ) PixelCN=84;
            else if(LC==22) PixelCN=86;
            else if(LC==23) PixelCN=87;
            else if(LC==24) PixelCN=92;
            else if(LC==101) PixelCN=84; // GREENROOF
            else if(LC==11 || LC==12 ||LC==90 || LC==95 || LC==92)PixelCN=100;
            else PixelCN=50;
            }
    ...
    {double PixelMan=-9.9;
                      if(soil==1)
                        { if(LC==11 || LC==12 ||LC==90 || LC==95 || LC==92)PixelMan=0.01;
                          ...
                          else if(LC==22) PixelMan=0.035;
                          else if(LC==23) PixelMan=0.035;
                          else if(LC==24) PixelMan=0.02;
                          else if(LC==31 || LC==32) PixelMan=0.04;
                          else if(LC==41 || LC==42 || LC==2000 || LC==43 ) PixelMan=0.08;
                          else if(LC==51 || LC==52) PixelMan=0.04;
                          else if(LC==71 || LC==72 || LC==73 || LC==74 )PixelMan=0.05;
                          else if(LC==81 || LC==61 )PixelMan=0.025;
                          else if(LC==82) PixelMan=0.03;
                          else if(LC==101) PixelMan=0.05; // GREENROOF
                          else PixelMan=0.03;
                      }

                      if(soil==2)
                        { if(LC==11 || LC==12 || LC==90 || LC==95 || LC==92)PixelMan=0.01;
                          else if(LC==21 || LC==83 || LC==84 || LC==85 ) PixelMan=0.07-0.0025;
                          else if(LC==22) PixelMan=0.035-0.0025;
                          else if(LC==23) PixelMan=0.035-0.0025;
                          else if(LC==24) PixelMan=0.02-0.0025;
                          else if(LC==31 || LC==32) PixelMan=0.04-0.0025;
                          else if(LC==41 || LC==42 || LC==2000 || LC==43 ) PixelMan=0.08-0.002;
                          else if(LC==51 || LC==52) PixelMan=0.04-0.0025;
                          else if(LC==71 || LC==72 || LC==73 || LC==74 )PixelMan=0.05-0.0025;
                          else if(LC==101 )PixelMan=0.05-0.0025;
                          else if(LC==81 || LC==61 )PixelMan=0.025-0.0025;
                          else if(LC==82) PixelMan=0.03-0.0025;
                          else PixelMan=0.03-0.0025;
                      }
                      if(soil==3)
                       { if(LC==11 || LC==12 ||LC==90 || LC==95 || LC==92)PixelMan=0.01;
                          else if(LC==21 || LC==83 || LC==84 || LC==85 ) PixelMan=0.07-0.005;
                          else if(LC==22) PixelMan=0.035-0.005;
                          else if(LC==23) PixelMan=0.035-0.005;
                          else if(LC==24) PixelMan=0.02-0.005;
                          else if(LC==31 || LC==32) PixelMan=0.04-0.005;
                          else if(LC==41 || LC==42 ||LC==2000 || LC==43 ) PixelMan=0.08-0.005;
                          else if(LC==51 || LC==52) PixelMan=0.04-0.005;
                          else if(LC==71 || LC==72 || LC==73 || LC==74 )PixelMan=0.05-0.005;
                          else if(LC==101 )PixelMan=0.05-0.005;
                          else if(LC==81 || LC==61 )PixelMan=0.025-0.005;
                          else if(LC==82) PixelMan=0.03-0.005;
                          else PixelMan=0.03-0.005;
                      }
                      if(soil==4)
                       { if(LC==11 || LC==12 ||LC==90 || LC==95 || LC==92)PixelMan=0.01;
                          else if(LC==21 || LC==83 || LC==84 || LC==85 ) PixelMan=0.07-0.0075;
                          else if(LC==22) PixelMan=0.035-0.0075;
                          else if(LC==23) PixelMan=0.035-0.0075;
                          else if(LC==24) PixelMan=0.02-0.0075;
                          else if(LC==31 || LC==32) PixelMan=0.04-0.0075;
                          else if(LC==41 || LC==42 || LC==2000 ||LC==43 ) PixelMan=0.08-0.0075;
                          else if(LC==51 || LC==52) PixelMan=0.04-0.0075;
                          else if(LC==71 || LC==72 || LC==73 || LC==74 )PixelMan=0.05-0.0075;
                          else if(LC==101 )PixelMan=0.05-0.0075;
                          else if(LC==81 || LC==61 )PixelMan=0.025-0.0075;
                          else if(LC==82) PixelMan=0.03-0.0075;
                          else PixelMan=0.03-0.0075;
                      }
