 ...
 */
public class NetworkEquations_AllMethod3 implements hydroScalingAPI.util.ordDiffEqSolver.BasicFunction {
    ...
    private float rr;
    private double qp, qpu, qpl, qus, qsl, Q_trib, K_Q;
    ...
    public NetworkEquations_AllMethod3(hydroScalingAPI.util.geomorphology.objects.LinksAnalysis links, hydroScalingAPI.modules.rainfallRunoffModel.objects.HillSlopesInfo hillinf, hydroScalingAPI.modules.rainfallRunoffModel.objects.LinksInfo linkIn, int connLink, java.io.File[] inputFiles, java.util.Hashtable rP) {
        ...
        for (int i = 0; i < lengthArray[0].length; i++) {
            ...
            Hill_SoilDepth[0][i] = (float) basinHillSlopesInfo.SCS_S2(i); // [mm] maximum storage - considering basin was dry
        ...
        switch (HillType) {
            case 0:    /* RUNOFF = RAINFALL - NO DELAY DELLAY FUNCTION */
                ...
                for (int i = 0; i < nLi; i++) {
                    ...
                    qp = 0;
                    qpu = 0;
                    qpl = effPrecip; //Uncomment to eliminate the effect of hillslope routing
                    qsl = 0.0;
                    ...
                    /* the links*/ output[i] = 60 * K_Q * ((1 / 3.6 * areasHillArray[0][i] * (qpl + qsl)) + Q_trib - input[i] - chanLoss); //[m3/s]/min
            ...
            case 1:    /* RUNOFF = RAINFALL - WITH DELLAY FUNCTION */
                ...
                for (int i = 0; i < nLi; i++) {
                   ...
                   if (rr == 1.0f) { // for cedar river iowa simulation
                    ...
                    effPrecip = Math.max(hillPrecIntensity - basinHillSlopesInfo.infiltRate(i, time), 0.0);
                    qp = effPrecip;
                    qpu = Math.max(0, hillPrecIntensity - effPrecip);
                    // Hillslope Velocity
                    ...
                    qpl = (input[i + nLi] / K); //mm/hour//((input[i+nLi] > So)?1:0)*(1/Ts*(input[i+nLi]-So));
                    ...
                    /* the links*/ output[i] = 60 * K_Q * (((1 / 3.6) * areasHillArray[0][i] * (qpl + qsl)) + Q_trib - input[i] - chanLoss); //[m3/s]/min
/* hillslope reservoir*/ output[i + nLi] = (1 / 60.) * (qp - qpl); //output[mm/min] qd and qs [mm/hour]
/* soil reservoir*/ output[i + 2 * nLi] = (1 / 60.) * (qpu - qsl); //output[mm/min] effPrecip and qss [mm/hour]
/* soil reservoir*/ output[i + 3 * nLi] = 0.0f; //output[mm/min] effPrecip and qss [mm/hour]
            ...
            case 3:
                ...
                for (int i = 0; i < input.length; i++) {
                    if (input[i] < 0.0 || Double.isNaN(input[i])) {
                        input[i] = 0.0;
                    }
                ...
                for (int i = 0; i < nLi; i++) {
                    ...
                    if (hrel >= 0 && hrel < hrelmax) {
                        if(hrel<=(Hill_SoilDepth[0][i]/1000)) ImpArea = 0;
                        ...
                        if(da_dh <0) da_dh=0; // area cannot decrease with h
                    ...
                    if (hillPrecIntensity > 0.0f && (Pe) > 0) {
                        ...
                        qp= hillPrecIntensity*areasHillArray[0][i]/3.6;    /// this would guarantee it is in m3/s
                        qpu = C*PermArea*(hillPrecIntensity)/3.6;          /// m3/s
                        if (qpu < 0) {
                            qpu = 0;
                        }
                    ...
                    qpl = (2 * (lengthArray[0][i] / (areasHillArray[0][i]))* (vr/3600)) * (input[i + nLi]*1e-3) *areasHillArray[0][i]; //m3/hour//((input[i+nLi] > So)?1:0)*(1/Ts*(input[i+nLi]-So));
                    double kunsat=Ksat*Math.exp(input[i + 3 * nLi]-1);
                    if(Double.isNaN(vr)) kunsat=Ksat;
                    qus = (kunsat/3600) *input[i + 3 * nLi]*(areasHillArray[0][i]-ImpArea)*1e6; //mm/hour
                    
                    if(ImpArea>0 && hrelmax>0)qsl = (Ksat/3600) * (ImpArea/areasHillArray[0][i])*((input[i + 2*nLi]-Hill_SoilDepth[0][i])/(hrelmax*1000))*PermArea*1e6;  //mm/hour // correct units!!!!!
                    else qsl =0;
...
/* the links*/                  output[i] = 60 * K_Q * (qpl + qsl + Q_trib - input[i]); //[m3/s]/min
                                if(Double.isNaN(output[i])) output[i]=0.0;
/*surface hillslope reservoir*/ output[i + nLi] = (60.) * ((qp - qpl)/(areasHillArray[0][i]*1e3)); //output[mm/h/min]
                                if(Double.isNaN(output[i + nLi])) output[i + nLi]=0.0;
/* depth - saturated area*/
                                double dV3_dt=(qus -qsl); //m3/h
                                if(da_dh>0 && Hill_SoilDepth[0][i]>0) output[i + 2 * nLi] =(60.) * (dV3_dt/(1e3*Hill_SoilDepth[0][i]*da_dh)) ; //output[mm/min] effPrecip and qss [mm/hour]
                                else output[i + 2 * nLi] =0;
                                if(Double.isNaN(output[i + 2 * nLi])) output[i + 2 * nLi]=0.0;
                                if(V2>0)
                                    ///////////////////////////////////
/* soil moisture*/              output[i + 3 * nLi] = ( 60.) * (1 / V2) * ((qpu - qus) - (input[i + 3 * nLi]) * dV3_dt); //output[1/min]
            ...
            case 4:
                ...
                for (int i = 0; i < nLi; i++) {
                    ...
                    if (hillPrecIntensity > 0.0f && (Pe) > 0) {
                        ...
                        qpu = C * hillPrecIntensity;    /// this would guarantee it is in mm/h
                        qp = hillPrecIntensity - qp;
                        if (qpu < 0) {
                            qpu = 0;
                        }
                    ...
                    qsl = 0.4 * (input[i + 2 * nLi] / K_subfast) + 0.6 * (input[i + 2 * nLi] / K_subslow);  //mm/hour
                    Q_trib = 0.0;
...
/* the links*/ output[i] = 60 * K_Q * ((1 / 3.6 * areasHillArray[0][i] * (qpl + qsl)) + Q_trib - input[i] - chanLoss); //[m3/s]/min
/* hillslope reservoir*/ output[i + nLi] = (1 / 60.) * (qp - qpl)/areasHillArray[0][i] ; //output[mm/min] qd and qs [mm/hour]
/* soil reservoir*/ output[i + 2 * nLi] = (1 / 60.) * (qpu - qsl); //output[mm/min] effPrecip and qss [mm/hour]
