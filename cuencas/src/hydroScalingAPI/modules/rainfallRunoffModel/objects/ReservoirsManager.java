...
//            resLocations[i]     =Integer.parseInt(resLoc.get(i));   
            maxStorages[i]      = 131644.1;
            maxDepths [i]       =5.03; //This has to always be larger than 1m plus orfice diameter
            weirLengths [i]     = 4;
             orficeDiameter[i]   = 0.75;
//            orficeDiameter[i]   = 0.75+0.152685*(Double.parseDouble(upsArea.get(i))-4.329947948);
//            System.out.println(resLocations[i]+", "+Double.parseDouble(upsArea.get(i))+","+orficeDiameter[i]);
...
//        resLocations = new int[]{2524};//{16,34}2524;
//        maxStorages=new double[] {466663.38};
//        maxDepths = new double[] {7.03}; //This has to always be larger than 1m plus orfice diameter
...
//            System.out.println("H: "+h);
            ...
            yield = NaturalConductivity*(storage-maxStorages[i]) + deltaT * 0.6 * 0.25 * Math.PI * Math.pow(orficeDiameter[i], 2) * Math.sqrt(2 * 9.81 * h)+deltaT * 3.1 * weirLengths[i] * Math.pow(h - (maxDepths[i] - 1), 1.5);
//            yield = upstreamYield + deltaT * 0.6 * 0.25 * Math.PI * Math.pow(orficeDiameter[i], 2) * Math.sqrt(2 * 9.81 * h)+deltaT * 3.1 * weirLengths[i] * Math.pow(h - (maxDepths[i] - 1), 1.5);
        }
