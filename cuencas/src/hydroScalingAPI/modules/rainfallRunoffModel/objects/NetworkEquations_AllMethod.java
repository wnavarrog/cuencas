 ...
 */
public class NetworkEquations_AllMethod implements hydroScalingAPI.util.ordDiffEqSolver.BasicFunction {
    ...
    private float rr;
    private double q01, q12, q1c, qe, Q_trib, K_Q, q3c, q_in_res, q_out_res;
     ...
     */
    public NetworkEquations_AllMethod(hydroScalingAPI.util.geomorphology.objects.LinksAnalysis links, hydroScalingAPI.modules.rainfallRunoffModel.objects.HillSlopesInfo hillinf, hydroScalingAPI.modules.rainfallRunoffModel.objects.LinksInfo linkIn, int connLink, java.io.File[] inputFiles, java.util.Hashtable rP) {
        ...
        Area_Relief_Param = new float[3][lengthArray[0].length];
        Hill_SoilDepth = new float[1][lengthArray[0].length];
        Hill_HillDepth=new float[1][lengthArray[0].length];
        Hill_TotalDepth = new float[1][lengthArray[0].length];
        ...
        for (int i = 0; i < lengthArray[0].length; i++) {
            ...
            Hill_HillDepth[0][i] = (float) basinHillSlopesInfo.HillRelief(i); // [mm] hillslope relief
            Hill_TotalDepth[0][i] = Hill_SoilDepth[0][i]+Hill_HillDepth[0][i] ; // [mm] hillslope relief
            Hill_TotalVol[0][i] =(float) (Hill_TotalDepth[0][i]*areasHillArray[0][i]*1e9); // hillslope relief
...
/* runoff rate*/            //output[i+3*nLi]=(1/60.)*q1c; // runoff rate mm/min
/* subsurface flow rate*/   //output[i+4*nLi]=(1/60.)*q3c; //subsurface flow mm/min
...
//the links
             ...
             case 3:
                ...
                for (int i = 0; i < nLi; i++) {
                    ...
                    if (hillPrecIntensity > 0.0f && (Pe) > 0) {
                        if (deficit <= 0) C = 1;
                        if (deficit >0) C = Pe * (Pe + 2 * deficit) / Math.pow((Pe + deficit), 2);
                        if (C > 1) C = 1;
                        if (C < 0) C = 0;
            ...
            case 4:
                ...
                for (int i = 0; i < nLi; i++) {
                    ...
                    q3c = 0.4*(input[i + 2 * nLi] / K_subfast)+ 0.6*(input[i + 2 * nLi] / K_subslow);  //mm/hour
                    Q_trib = 0.0;
...
/* accum prec*/             //output[i+5*nLi]=(1/60.)*hillPrecIntensity; //accumulated precipitation mm/min
    ...
    {
    double vr = 1.0;
     double Slope = Math.max(0.005, slopesArray[0][hilslope]);
    int i=hilslope;
    int nLi = linksConectionStruct.connectionsArray.length;
                    switch (HillVelType) {
                        case 0: //constant
                        ...
                        case 1: //manning equation - manning roughness as a function of land cover and soil hyd group
                            vr = (1 / basinHillSlopesInfo.HillManning(hilslope)) * Math.pow(Slope, 0.5) * Math.pow(SurfDepth / 1000, (2 / 3)) * 3600; //(m/h)
                            break;
