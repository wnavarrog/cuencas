 ...
 */
public class NetworkEquations_HillDelayPlusLinearHillslopeChi implements hydroScalingAPI.util.ordDiffEqSolver.BasicFunction {
     ...
     */
    public NetworkEquations_HillDelayPlusLinearHillslopeChi(hydroScalingAPI.util.geomorphology.objects.LinksAnalysis links, hydroScalingAPI.modules.rainfallRunoffModel.objects.HillSlopesInfo hillinf, hydroScalingAPI.modules.rainfallRunoffModel.objects.LinksInfo linkIn, int rt,java.util.Hashtable extraParameters){
        ...
        generalRunoffCoeff=new double[linksConectionStruct.connectionsArray.length];
        java.util.Arrays.fill(generalRunoffCoeff, 0.50);
        
        int[] linkIDsGauges=(int[])extraParameters.get("linkIdsStat");
        double[] runoffCoeffs=(double[])extraParameters.get("runoffCoeff");
        
        resetRunoffCoefficients(linkIDsGauges,runoffCoeffs);

    }
    
    public void resetRunoffCoefficients(int[] linkIDsGauges, double[] runoffCoeffs){
        for (int i = 0; i < runoffCoeffs.length; i++) {
            assignRunoff(linkIDsGauges[i]-1,runoffCoeffs[i],linksConectionStruct.connectionsArray);
        }
    }
    
    private void assignRunoff(int linkID,double runoffCoeff,int[][] conArr){
        generalRunoffCoeff[linkID]=runoffCoeff;
        for (int i = 0; i < conArr[linkID].length; i++) {
            assignRunoff(conArr[linkID][i],runoffCoeff,conArr);
        }
    }
    ...
    /**
     * The actual evaluate method for this function.  The equation implemented here is
     * 
     * dq/dt=sum(q_trib)-q
    ...
    public double[] eval(double[] input, double time) {
        ...
        for (int i=0;i<nLi;i++){
            ...
            evaporation=input[i+2*nLi]>5?(3/24.0):0;
            
            double k3=1/30./24.0;

            double qsub=k3*input[i+2*nLi];//k2/290.0*input[i+2*nLi];
            ...
            output[i+linksConectionStruct.connectionsArray.length]=1/60.*(generalRunoffCoeff[i]*hillPrecIntensity-qs);

            //the hillslope storage (soil moisture and redistribution)
            output[i+2*linksConectionStruct.connectionsArray.length]=1/60.*((1-generalRunoffCoeff[i])*hillPrecIntensity-qsub-evaporation);
