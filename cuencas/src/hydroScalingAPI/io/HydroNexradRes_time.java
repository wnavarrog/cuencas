...
// Read all the files and attribute values to matrixtime[][][]   
       //System.out.println("Start to read the files"+"ListFiles.length = " + ListFiles.length); 
        ...
        for(int il=0;il<ListFiles.length;il++){    
            ...
            // System.out.println("calculation tt =" + tt);
...
// calculate the final values        
    ...
    if (type.equals("Bin")){vhcFilename="prec."+df.format(time[3])+df.format(time[4])+"00."+df.format(time[2])+"."+monthString+"."+df2.format(time[0])+".vhc";}
    else {vhcFilename="prec."+df.format(time[3])+df.format(time[4])+"00."+df.format(time[2])+"."+monthString+"."+df2.format(time[0])+".asc";}
    return vhcFilename;
    }
   ...
   timeStamp[0]=fileName.substring(20,24); // year
   time[0]=Integer.parseInt(timeStamp[0]); 
   timeStamp[1]=fileName.substring(15,18);  // month   
   ...
   for (int ii=0;ii<11;ii++)
    ...
    timeStamp[2]=fileName.substring(12,14);  // day
    time[2]=Integer.parseInt(timeStamp[2]);
    timeStamp[3]=fileName.substring(5,7); // hour 
    time[3]=Integer.parseInt(timeStamp[3]);
    timeStamp[4]=fileName.substring(7,9); // min
    time[4]=Integer.parseInt(timeStamp[4]);
    timeStamp[5]=fileName.substring(9,11); //seg
