...
public class HydroNexradToCRasSatelliteData extends Object {
    ...
    public HydroNexradToCRasSatelliteData(String Satellite, java.io.File inputFile, java.io.File outputFile, int numRow, int numCol, float coef) throws java.io.IOException {
    ...
    public static ArrayList<File> getFileList(File dir, String fil) throws FileNotFoundException {
        ...
        for (File file : files) {
            if (file.getAbsoluteFile().toString().contains(fil)) {
                result.add(file);
            }
        }
        return result;
    ...
    public static void createMetaFile(String Satellite, File directory, double xxcorner, double yycorner, double newsresol, int newtresol, int Finalrows, int Finalcolumns, String rain) {
        try {
            ...
            int degree = (int) Math.floor(yycorner);
            int Min = (int) Math.floor((yycorner - degree) * 60);
            double Sec = (((yycorner - (double) degree) * 60.f) - (double) Min) * 60.f;
            String str = degree + ":" + Min + ":" + Sec + " N";
            writer.println(str); // Iowa River
            ...
            degree = (int) Math.floor(Math.abs(xxcorner));
            Min = (int) Math.floor((Math.abs(xxcorner) - degree) * 60);
            Sec = (((Math.abs(xxcorner) - (double) degree) * 60.f) - (double) Min) * 60.f;
            str = degree + ":" + Min + ":" + Sec + " W";
            ...
            writer.println(Satellite + "Provided by Hyun");
            writer.close();
    ...
    public static String OutfilenamePER(String fileName, String ident) {
...
//        timeStamp[4]=fileName.substring(30,32); // min
        int index = fileName.lastIndexOf("_3h") + 3;
...
//        }
    ...
    public static String OutfilenameTRMM(String fileName, String ident) {
        System.out.println("fileName - FROM");
        System.out.println(fileName);
        String[] months = {"January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"};
        String[] months2 = {"JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"};
        ...
        int index = fileName.lastIndexOf("42.") + 3;
        System.out.println("index    =     " + index);
        timeStamp[0] = "20" + fileName.substring(index, index + 2); // year
        System.out.println(timeStamp[0]);
        timeStamp[1] = fileName.substring(index + 2, index + 4);  // month
        ...
        timeStamp[2] = fileName.substring(index + 4, index + 6);  // day
        System.out.println(timeStamp[2]);
        timeStamp[3] = fileName.substring(index + 7, index + 9); // hour
        System.out.println(timeStamp[3]);
        timeStamp[4] = "00"; // min
    ...
    public static String OutfilenameCMORPH(String fileName, String ident) {
        System.out.println("fileName - FROM");
        System.out.println(fileName);
        String[] months = {"January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"};
        String[] months2 = {"JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"};
...
//        System.out.println(timeStamp[3]);
//        timeStamp[4]=fileName.substring(30,32); // min
        int index = fileName.lastIndexOf("PH_") + 3;
        System.out.println("index    =     " + index);
        timeStamp[0] = fileName.substring(index, index + 4); // year
        System.out.println(timeStamp[0]);
        timeStamp[1] = fileName.substring(index + 4, index + 6);  // month
...
//        timeStamp[4]=fileName.substring(30,32); // min
    ...
    public static void subMainALL(String[] args) throws java.io.IOException {
        ...
        String filter = "m6s4";
          int ncols = -9;
        int nrows = -9;
        double xllcorner = -97.125;
        double yllcorner = 40.125;
        double cellsize = 0.25;
        double Sres = 0.25 * 3600;
        float units=1;//mm/h
        String Satellite = "TRMM_V6";
        if (Satellite.contains("PERSIANN")) {
            filter = "m6s4";
            ens = "m6s4.";
            ncols = 30;
            nrows = 18;
        ...
        int TRes = 180;
        float bias = 1.00f;
        float coef = (60.f / (float) TRes) * bias*units;
 
        for (int iy = 2008; iy < 2009; iy++) {
            java.io.File folder = new java.io.File("/scratch/Users/rmantill/CuencasDataBases/Iowa_Rivers_DB/Rasters/Hydrology/storms/observed_events/SatelliteData/" + Satellite + "/" + iy + "/");
            System.out.println(folder.toString());


            ArrayList<File> files = HydroNexradToCRasSatelliteData.getFileList(folder, filter);
...
//        String OutputDir="/scratch/CuencasDataBases/Iowa_Rivers_DB/Rasters/Hydrology/storms/observed_events/1996_2010RadarLowRes/1996VHC/";         String OutputDir="/Users/rmantill";
            java.io.File outputDirectory = new java.io.File("/scratch/Users/rmantill/CuencasDataBases/Iowa_Rivers_DB/Rasters/Hydrology/storms/observed_events/SatelliteData/" + Satellite + "VHCv3/" + iy + "/");

            System.out.println(outputDirectory.toString());
            outputDirectory.mkdirs();


            createMetaFile(Satellite, outputDirectory, xllcorner, yllcorner, Sres, TRes, nrows, ncols, ens);
            while (i.hasNext()) {
                ...
                File BinaryOutName = new java.io.File("null");
                if (Satellite.contains("PERSIANN")) {
                    BinaryOutName = new java.io.File(outputDirectory.getAbsoluteFile().toString() + File.separator + OutfilenamePER(fileName, ens));
                }
                if (Satellite.contains("CMORPH")) {
                    BinaryOutName = new java.io.File(outputDirectory.getAbsoluteFile().toString() + File.separator + OutfilenameCMORPH(fileName, ens));
                }
                if (Satellite.contains("TRMM")) {
                    BinaryOutName = new java.io.File(outputDirectory.getAbsoluteFile().toString() + File.separator + OutfilenameTRMM(fileName, ens));
                }
                ...
                try {
                    System.out.println(temp.getAbsolutePath());
                    new HydroNexradToCRasSatelliteData(Satellite, AsciiFile, BinaryOutName, nrows, ncols, coef);
                } catch (Exception IOE) {
